...
type SCIMGroupAdapter = Pick<
	DBAdapter,
	"create" | "delete" | "deleteMany" | "findMany" | "findOne" | "update"
>;
...
async function createGroupMemberReferences(
	...
	return groupMembers
		.map((member) => {
			...
			return {
				value: user.id,
				$ref: getResourceURL(`/scim/v2/Users/${user.id}`, ctx.context.baseURL),
...
async function replaceGroupRoleGrants(
	...
	for (const userId of input.userIds) {
		const member = await findOrganizationMember(adapter, {
			organizationId: input.group.organizationId,
			userId,
		});
		if (!member) {
		...
		for (const role of input.roles) {
			...
			const isRoleProjected = await addProjectedRole(adapter, {
				member,
				role,
			});
...
export async function listSCIMGroupResources(
	...
	const provider = getProviderScope(ctx);
	const filter = query?.filter ? parseGroupFilter(query.filter) : undefined;
	...
	const resources = groups.slice(startIndex - 1, startIndex - 1 + count);
	const groupViews = await Promise.all(
		resources.map((group) => createGroupView(ctx, group)),
	);
...
function parseGroupFilter(filter: string) {
	const match = filter.match(
		/^\s*displayName\s+eq\s+(?:"(?<quoted>[^"]*)"|(?<bare>[^\s]+))\s*$/i,
	);
	if (!match) {
		throw new SCIMAPIError("BAD_REQUEST", {
			detail: "Only displayName eq filters are supported for SCIM Groups",
			scimType: "invalidFilter",
		});
	}
	return {
		displayName: match.groups?.quoted ?? match.groups?.bare ?? "",
	};
}
...
function applyGroupPatchObject(
	...
	if (typeof patchValue.displayName === "string") {
		state.displayName = patchValue.displayName;
	}
...
function applyGroupPatchOperation(
	...
	if (path === "displayName") {
		...
		state.displayName = getPatchString(operation.value, "displayName");
		return;
	}
